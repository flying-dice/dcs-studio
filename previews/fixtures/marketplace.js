...
(() => {
  ...
  const products = {
    "viper-drivers/f16-weapons-expansion": {
      ...
      missionScripts: [
        {
          name: "Rearm menu hook",
          purpose: "Adds stores to the ground-crew rearm menu",
          path: "Scripts/WeaponsExpansion/rearm.lua",
          run_on: "after-sanitize",
        },
        {
          name: "HARM table injector",
          purpose: "Patches emitter tables at load",
          path: "Scripts/WeaponsExpansion/harm.lua",
    ...
    "dcs-scripting/moose-lite": {
      ...
      assets: [
        { name: "moose-lite-v0.9.0.zip", size: 629145.6 },
        { name: "dcs-studio.toml", size: 614.4 },
      ],
      download_size: 629145.6,
      installable: true,
      ...
      missionScripts: [
        {
          name: "MOOSE loader",
          purpose: "Boots the framework at mission start",
          path: "Scripts/MooseLite/loader.lua",
          run_on: "after-sanitize",
        },
      ],
      requires: [],
    ...
    "sound-mods/immersive-cockpit-audio": {
      ...
      stars: 98,
      release_tag: "v1.1.0",
      release_url: "https://github.com/sound-mods/immersive-cockpit-audio/releases/tag/v1.1.0",
      release_date: new Date(Date.now() - 12 * 86400000).toISOString(),
      readme: "# Immersive Cockpit Audio\n\nDrop-in sound folder, no scripting.\n",
      assets: [
        { name: "immersive-cockpit-audio-v1.1.0.zip", size: 88080384 },
        { name: "dcs-studio.toml", size: 400 },
      ],
      download_size: 88080384,
      installable: true,
    ...
    "mission-makers/operation-eastern-storm": {
      ...
      assets: [
        { name: "operation-eastern-storm-1.4.0.zip", size: 134217728 },
        { name: "dcs-studio.toml", size: 921.6 },
      ],
      download_size: 134217728,
      installable: true,
      installs: [
        {
          source: "Campaigns/EasternStorm",
          dest: "Saved Games/DCS/Missions/Campaigns/EasternStorm",
        },
      ],
      requires: [
    ...
    "utils/dcs-lua-common": {
      ...
      stars: 67,
      release_tag: "v0.4.0",
      release_url: "https://github.com/utils/dcs-lua-common/releases/tag/v0.4.0",
    ...
    "training/bfm-trainer": {
      ...
      stars: 201,
      release_tag: "v3.0.0",
      release_url: "https://github.com/training/bfm-trainer/releases/tag/v3.0.0",
  ...
  // `delay` is scripted latency — the specs drive the busy/progress states
  // through it. With no delay the reply goes straight out: previews/harness.js
  // already fans postMessage out in a later task, so nothing here needs its own
  // timer to stay off the poster's stack.
  function reply(msg, delay) {
    if (delay) setTimeout(() => window.__host.receive(msg), delay);
    else window.__host.receive(msg);
  }
  ...
  function productFor(repo) {
    ...
    return {
      ...
      readme: `# ${l.name}\n\n${l.description}`,
      release_tag: "v1.0.0",
      release_url: `${l.repo_url}/releases`,
      release_date: new Date(Date.now() - 40 * 86400000).toISOString(),
      assets: [
        { name: `${l.name.replace(/\s+/g, "-").toLowerCase()}.zip`, size: 6291456 },
        { name: "dcs-studio.toml", size: 900 },
      ],
  ...
  function symlinkEscapes(s) {
    const out = [];
    if (!destStaysUnder(s.dest))
      out.push(unsafe("symlink-dest", "Link destination", "the configured DCS folders", s.dest));
    if (!staysUnder(s.source))
      out.push(unsafe("symlink-source", "Link source", "the mod's own folder", s.source));
    return out;
  }
  function entrypointEscapes(e) {
    const out = [];
    if (!staysUnder(e.exe))
      out.push(unsafe("entrypoint-exe", "Executable", "the mod's own folder", e.exe));
    if (e.cwd && !staysUnder(e.cwd))
      out.push(
        unsafe("entrypoint-cwd", "Executable working directory", "the mod's own folder", e.cwd),
      );
    return out;
  }
  function missionScriptEscapes(m) {
    return staysUnder(m.path)
      ? []
      : [unsafe("mission-script-path", "Mission script", "the mod's own folder", m.path)];
  }
  ...
  window.__host.onPost((m) => {
    ...
    switch (m.type) {
        ...
        reply(
          {
            type: "product",
            product,
            manifest: manifestFor(product),
            requires: requiresFor(product),
            installed: installed.has(repo),
          },
          450,
        );
        break;
      ...
      case "install": {
        const repo = m.repo;
        reply(
          { type: "installProgress", repo, phase: "download", label: "Downloading…", pct: 0.15 },
          50,
        );
        reply(
          { type: "installProgress", repo, phase: "download", label: "Downloading…", pct: 0.7 },
          100,
        );
        reply(
          { type: "installProgress", repo, phase: "link", label: "Linking into DCS…", pct: 1 },
          130,
        );
        setTimeout(() => {
          installed.add(repo);
