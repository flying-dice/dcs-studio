...
- **DCS Console** in the app: bottom tool window (`src/lib/components/LuaConsole.svelte`) — response-only output of every run; runs come from the editor's "Run in DCS" (file or selection, `src/lib/lua-console.svelte.ts` `eval`/`runFile`). The standalone `/console` route and the REPL retired with issue #47.
- `dcsCall` (`src/lib/api.ts`) routes through the Rust dcs-bridge-client under Tauri, and **falls back to a direct browser WebSocket** (`src/lib/dcs-ws.ts`) when running in plain `vite dev` — so the UI is usable against live DCS without the Tauri shell.

## Live test suites (run with the sim up)

The old `e2e/` Playwright suite retired with `/console` (no specs remained).
Live verification is now `#[ignore]`d Rust integration tests, run explicitly
once DCS is up — the durable pattern (each skips hermetically by default):

```powershell
cargo test -p studio-services --test live_types -- --include-ignored   # type sync round-trip (issue #50 DoD)
cargo test -p dcs-bridge -- --include-ignored                          # DLL surface vs golden (needs lua.dll on PATH, see below)
```

For `dcs-bridge` tests on Windows, put DCS's own `lua.dll` on PATH first:
