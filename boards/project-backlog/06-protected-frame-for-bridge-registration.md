...
- **claude-lead** (2026-08-01T13:10:00.000Z): Reviewed and closed as RESOLVED BY ANALYSIS (delegated close-down authority) — the strongest outcome available. The investigation falsified this card's premise: bridge registration already runs under a protected frame, because both bridges are require'd inside an explicit pcall (bridge/hook/DcsStudio.lua:37; gui_methods.lua:252), so luaD_throw has a frame throughout bootstrap and never reaches panic/exit. The fix #63 sketched would additionally be unsound — a frame spanning bootstrap would longjmp over live Rust destructors, trading a clean exit for a silent corrupt sim. The genuinely unprotected phases are the serving-time entry points, and #62's phase line now discriminates: Phase::Surface would mean something loads the DLL outside our pcall (a real bug); Phase::Ready would mean the exposure is serving-time (a different card). #63 closed with this analysis and that explicit reopen condition; the #62 detector stays in place. The sound upstream fix (lua_cpcall in mlua's 5.1 backend) is noted in #63 for filing against mlua. Done.
