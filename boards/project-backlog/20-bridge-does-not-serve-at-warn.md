...
- [x] Live-verify both bridges serve at the shipped level — **done 2026-07-30: both serve at `warn`, GUI on two launches, mission on two missions**
...
- **opus-live2** (2026-07-30T05:00:00.000Z): **Live-verified — PASS on both bridges at the shipped `warn`, and the card is closed.** Deployed from the tree with no DCS running (`bridge/deploy/deploy.ps1`, both DLLs + hook, both `logger_level` literals confirmed `warn` beforehand at `bridge/hook/DcsStudio.lua:35` and `bridge/crates/bridge-core/lua/gui_methods.lua:240` — no workaround to revert, so this tested the shipped configuration as-is). **GUI bridge (`:25569`):** `GET /health` returned **200** `{"name":"dcs-studio-gui","env":"gui","status":"OK","version":"0.3.0"}` ~32 s after launch, and an `eval` round-trip returned a real value (`return 2+2, _VERSION, …` → `4`). `netstat` showed `127.0.0.1:25569 LISTENING` — the exact opposite of this card's failing signature, which was *no listener at all* while the hook logged its success line. Reproduced on **two separate launches** (00:47:42 and 00:54:14 local), matching the two launches that failed before the fix. **Mission bridge (`:25570`):** `GET /health` returned `{"name":"dcs-studio-mission","env":"mission","status":"OK","version":"0.3.0"}` with `25570 LISTENING`, and `eval` returned `timer.getTime()` = **17.4** (run 1) and **10.5** (run 2) — a genuine round-trip into the mission Lua state, on **two separate missions**. Then 36/36 and 40/40 mixed `eval`/`debug_state`/`console_read`/`ping` requests served with **zero failures**. So the bridge does not merely bind at `warn`, it serves, on both DLLs independently — the fix at `bridge/crates/bridge-core/src/jsonrpc/server.rs:156-252` holds inside DCS exactly as the headless regression test predicted.
- **opus-live2** (2026-07-30T05:02:00.000Z): One measurement worth recording for whoever reads the old evidence table: the first mission-bridge `eval` of run 1 **did** time out at the full 30 s, and it is *not* a residue of this bug — the mission had loaded but was still sitting on the BRIEFING screen, so model time was frozen and the 0.1 s-model-time pump (`bridge/crates/bridge-mission/lua/mission_init.lua:77-85`) never drained the queue. `GET /health` answered fine throughout. Clicking FLY made the same port answer `eval` immediately. That is card 17 / card 04's "reachability is not liveness" signature, not a bind failure, and it is worth knowing because at a glance it looks identical to this card's fault. The discriminator is `netstat`: this card's failure showed **no listener**; a frozen pump shows `LISTENING` plus a healthy `/health`. Card moved to **review** — live verification was the only item outstanding, and it passed on both bridges, twice each. No DCS left running; the deployment stays in place as the current tree. — a narrow race that defeated the timeout's own purpose. The `Timeout` arm returned `Err` while `outcome_rx` was still alive for the rest of the scope, so a `send(Ok(handle))` landing in that window would SUCCEED, the server thread's "nobody left to serve for" release would never fire, and the bridge would serve forever on a handle nothing could reach — a port bound to nothing for the rest of the DCS session, precisely the leak the bounded wait exists to prevent. `drop(outcome_rx)` is now the first statement of that arm (`bridge/crates/bridge-core/src/jsonrpc/server.rs:236-244`), which makes a late send fail by construction, with a comment naming the race. Also took the cosmetic: `host_for_error` is gone, the two error `format!`s reference `config.host` directly (`server.rs:245-249`, `server.rs:255-258`) since `config` is not moved until `Ok(Self { .. })`. Re-gated: `cargo fmt --all -- --check` clean, `cargo clippy --all-targets -- -D warnings` clean, `cargo test --workspace` green (35 passed, 94 Windows-ignored), bridge-core lib **127/127** and `jsonrpc_server` **9/9** with DCS's `lua.dll` on PATH, and both card-20 regression tests re-run green by name. Live-verify still unticked; still no DCS launched and no `Saved Games` touched.
- **claude-lead** (2026-07-30T02:20:00.000Z): Reviewed and approved (review delegated by the owner). Root cause proven by construction (server.await inside an info! argument, compiled out below info) with a regression test verified failing pre-fix; the review pass added the timeout-arm receiver drop closing the stranded-listener window; the fix also exonerated this machine's socket tests (jsonrpc_server 1/9 → 9/9). Live-verified in session 3 prep: both bridges serving at the shipped warn on two launches / two missions, 36/36 + 40/40 requests. Done.
