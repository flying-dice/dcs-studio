...
| 10 | [A dedicated MissionScripting panel](10-where-missionscripting-is-managed.md) | Accepted | 2026-08-05 | unblocks card 41 | A panel named for the file it owns, because only a real surface can show the unmanaged content in a file users hand-edit. |
| 11 | [Two ways to run: the buffer, and the built project](11-two-ways-to-run.md) | Accepted | 2026-08-05 | unblocks card 42, uses [03](03-install-mods-as-links-not-copies.md) | Right-click Run keeps sending the buffer; a separate Run project builds and installs like a real release first; `dofile`/`require` detection points you between them. |
| 12 | [DCS Studio does not participate in editor lint](12-no-participation-in-editor-lint.md) | Accepted | 2026-08-05 | unblocks card 43 | Neither route: no emitted editor config, no `[lint]` section, no mirroring. luabox owns this, and `[lint]` stays an ordinary unknown section. |
| 13 | [The State Explorer becomes its own panel](13-explorer-as-its-own-panel.md) | Accepted | 2026-08-05 | unblocks #75/#76 on card 45 | Separate panels, and separate means the console keeps no explorer tab — no deprecation window, because one explorer cannot half-sync with another. |
| 14 | [How Copy Deep resolves a subtree](14-how-copy-deep-resolves-a-subtree.md) | Accepted | 2026-08-05 | unblocks card 45, follows [13](13-explorer-as-its-own-panel.md) | Client-side sweep: at the 32MB/30s wall it degrades with a truthful partial, where a single-shot serialize can only fail. |
