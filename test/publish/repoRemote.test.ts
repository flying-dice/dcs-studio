...
describe("parseRepoRemote", () => {
  ...
  it("parses an https remote without a .git suffix", () => {
    expect(parseRepoRemote("https://github.com/owner/repo")).toEqual({ owner: "owner", name: "repo" });
  });

  it("parses an ssh remote with a .git suffix", () => {
    expect(parseRepoRemote("git@github.com:owner/repo.git")).toEqual({ owner: "owner", name: "repo" });
  });
  ...
  it("parses an ssh:// protocol remote", () => {
    expect(parseRepoRemote("ssh://git@github.com/owner/repo.git")).toEqual({ owner: "owner", name: "repo" });
  });

  it("matches github.com case-insensitively", () => {
    expect(parseRepoRemote("https://GitHub.COM/Owner/Repo.git")).toEqual({ owner: "Owner", name: "Repo" });
  });
  ...
  it("keeps dots in the repo name while dropping only the trailing .git", () => {
    expect(parseRepoRemote("https://github.com/owner/my.mod.git")).toEqual({ owner: "owner", name: "my.mod" });
  });
