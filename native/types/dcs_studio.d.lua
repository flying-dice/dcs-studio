...
--- Post a payload (JSON string) under `key` in the process-wide mailbox, replacing any unclaimed previous value. Used to forward mission-bound requests and to return their results.
---@param key string
---@param value string
function dcs_studio_debug.post_box(key, value) end

--- Take (and remove) the mailbox payload under `key`, or nil.
---@param key string
---@return string? value
function dcs_studio_debug.take_box(key) end

--- Mark the resident mission-side runtime as available (set by its bootstrap) or gone (cleared by the hook on onSimulationStop).
---@param ready boolean
function dcs_studio_debug.set_mission_ready(ready) end

--- Whether the resident mission-side runtime is available to serve forwarded mission requests.
---@return boolean ready
function dcs_studio_debug.mission_ready() end
...
--- Drain the running server's queued requests through `router`, callable from ANY Lua state in the process (not just the one that owns the server). The mission-state debugger pumps the editor's requests with this while a paused chunk holds the sim thread. Returns false when no server is running.
---@param router dcs_studio.jsonrpc.JsonRpcRouter
---@return boolean served
function dcs_studio_jsonrpc.process_queue(router) end
