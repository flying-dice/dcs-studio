...
/// The running server's request queue, reachable process-wide so ANY Lua state
/// in the process can drain it — not just the GameGUI hook that owns the
/// `JsonRpcServer` userdata. The mission debugger depends on this: while a
/// mission-state chunk is paused at a breakpoint the sim thread is blocked
/// inside that state, the hook's `onSimulationFrame` never fires, and the only
/// code that can answer the editor's resume/inspect requests is the mission
/// state's own pause loop, via [`process_global_queue`].
...
fn drain_queue(
    ...
    for app_request in queue {
        respond(lua, router, app_request);
    }
