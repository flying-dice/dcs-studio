...
| `model/studio/cargolua.pds` | `Git` external system + `DependencyResolver`/`Bundler` containers — the **lua-cargo** toolchain: a Cargo-shaped git dependency manager (`github = "owner/repo"` deps vendored into `.lua-cargo/deps/`, pinned by branch/tag/rev, locked in `CargoLua.lock`) and a require-graph bundler/amalgamator (walk `require("mod")` from a `[[bundle]]` entry → one self-contained Lua 5.1 file behind a `__require` shim, sources verbatim, unresolved requires are warnings). Lean crate, no dcs-studio-project dep (issue #48 Part A; `crates/lua-cargo` lib + `lua-cargo` CLI: `build`/`fetch`/`bundle`) |
...
| `model/studio/bookmarks.pds` | `Bookmarks` — per-project file:line marks behind the left-rail Bookmarks panel: gutter toggle, click-to-jump, edit-tolerant anchoring (re-map through CM transactions while open, write back on save), localStorage-persisted by canonical root (`src/lib/bookmarks.svelte.ts`, `src/lib/editor/bookmark-gutter.ts`, `src/lib/components/Bookmarks.svelte`) |
