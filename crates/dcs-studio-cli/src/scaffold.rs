//! `dcs-studio-cli init` — materialise a template
//! (model: `studio::cli::Cli.Init`).
...
pub fn init(template: &str, parent: &Path, name: &str) -> Result<PathBuf, String> {
    let files = crate::templates::render(template, name)
        .ok_or_else(|| format!("unknown template '{template}' (try: lua-script, blank)"))?;
...
mod tests {
    ...
    fn temp_root(tag: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("dcs-cli-test-{tag}-{}", std::process::id()));
