...
- ~~**G3's declared message-contract table.**~~ **Since done, for six of eleven
  ...
  `log`, `publish` and `newproject`, the panels with a presenter — and checked from three directions: the compiler
  ...
  way past it was to observe the webview rather than read it. The other five
...
- ~~**Presenter extraction beyond the marketplace pilot.**~~ **Since done**
  ...
  covered by the unit layer rather than the integration one. `logPanel` and
  `publishPanel` followed the same way (board card
  `14-presenter-rollout-remaining-panels`): a 130-line shell over
  `core/app/logPresenter.ts`, and a 90-line shell over
  `core/app/publishPresenter.ts`; `newProjectPanel` is a 115-line shell over
  `core/app/newProjectPresenter.ts`. The G5 table above
