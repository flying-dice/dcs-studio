...
- ~~**G3's declared message-contract table.**~~ **Since done, for five of eleven
  ...
  `log` and `publish`, the panels with a presenter — and checked from three directions: the compiler
  ...
  way past it was to observe the webview rather than read it. The other six
...
- ~~**Presenter extraction beyond the marketplace pilot.**~~ **Since done**
  ...
  `core/app/publishPresenter.ts`. The G5 table above
