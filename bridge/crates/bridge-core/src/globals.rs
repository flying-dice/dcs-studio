...
//! [`crate::luadef`], golden-tested on any platform.

use std::collections::HashSet;
use std::ffi::c_void;

use crate::luadef::{emit_globals_dlua, is_emittable_segment, GlobalKind, GlobalNode, ScalarTy};
...
/// Introspect the curated DCS API roots (`curated_roots` — per bridge kind,
/// see [`crate::BridgeKind`]) present in `_G` and emit them as dotted `.d.lua`
/// statements. An absent root is skipped, not errored; a read failure yields
/// the partial dump gathered so far. Never raises in the sim.
pub fn dump_globals(lua: &Lua, curated_roots: &[&str]) -> String {
...
mod tests {
    ...
    fn dumps_curated_roots_and_bounds_the_walk() {
        ...
        let out = super::dump_globals(&lua, &["DCS", "Export", "net", "lfs", "log"]);
