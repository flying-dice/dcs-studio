...
impl Sub<'_> {
    /// The fully qualified `.d.lua` class name for `type_name` inside this
    /// sub-namespace (e.g. `dcs_studio_gui.jsonrpc.JsonRpcRouter`) — for doc
    /// type strings that must track the parametrized module root.
    pub fn qualified(&self, type_name: &str) -> String {
        format!("{}.{type_name}", self.full_name)
    }
