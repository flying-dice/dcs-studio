...
const META_SCHEMA: &str =
    include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/tests/fixtures/openrpc-meta-schema.json"));
...
fn assert_valid_openrpc(kind: BridgeKind) {
    ...
    let doc_json = emit_openrpc_json(kind, env!("CARGO_PKG_VERSION")).expect("generate openrpc doc");
    let doc: serde_json::Value = serde_json::from_str(&doc_json).expect("openrpc doc is valid JSON");
