...
return function(router, deps)
  ...
  local DBG = deps.DBG
  local RT = deps.RT -- the console runtime (__DCS_STUDIO_RT); owns the shared print/envelope/export helpers
  ...
  -- print() output streams into the console ring via RT.with_print_capture (the
  -- shared shim, with bridge.console.print as the sink).
  ...
  -- Run the rt call in `envname` (rt_call) then decode its JSON envelope via the
  -- shared RT.decode_envelope, which forwards any remote print() output riding in
  -- the envelope to the console ring so it streams to the IDE like local prints.
  -- (The mission bridge has no env dimension, so it calls RT.decode_envelope
  -- directly on its local RT.*_json result.)
  local function rt_call_decoded(envname, callexpr)
    return RT.decode_envelope(
      bridge.json.decode,
      bridge.console.print,
      rt_call(envname, callexpr),
      "'" .. envname .. "'"
    )
  end
  ...
  router:add_method("repl_expand", function(params)
    local envname = repl_env(params)
    return rt_call_decoded(envname, string.format("expand_json(%d)", (params and params.ref) or 0))
  end, {
  ...
  router:add_method("repl_signature", function(params)
    local envname = repl_env(params)
    return rt_call_decoded(envname, string.format("signature_json(%d)", (params and params.ref) or 0))
  end, {
  ...
  router:add_method("repl_clear", function(params)
    return rt_call_decoded(repl_env(params), "clear_json()")
  end, {
  ...
  local function mission_boot_source()
    ...
    -- mission_init.lua's report_error/report_info. It can't be factored out: this
    -- snippet runs in the bare mission trigger sandbox BEFORE the mission DLL (and
    -- thus rt.lua/bridge) exists there, so there is nothing shared to call. Keep
    -- the two literals identical by hand.
...
if not __DCS_STUDIO_MISSION_BOOTED then
  ...
  -- The unit-db curation library lives in gui_db.lua (GUI_DB), composed ahead of
  -- this chunk by load_register_methods. Bind its functions/constants as locals
  -- so the db_* handlers below read as thin wiring — the interpretation of the
  -- raw db graph is all over there.
  local need_db = GUI_DB.need_db
  local get_cache = GUI_DB.get_cache
  local type_index = GUI_DB.type_index
  local sanitize = GUI_DB.sanitize
  local attribute_names = GUI_DB.attribute_names
  local perf_fields = GUI_DB.perf_fields
  local pylons_of = GUI_DB.pylons_of
  local DB_CAP = GUI_DB.DB_CAP
  local RAW_MAX_DEPTH = GUI_DB.RAW_MAX_DEPTH
