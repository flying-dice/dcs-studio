...
---@class dcs_studio_gui.json
local dcs_studio_gui_json = {}
...
---@return string? json
---@return string? err
function dcs_studio_gui_json.encode(value, opts) end
...
---@return string? json
---@return string? err
function dcs_studio_gui_json.safe_encode(value) end

--- Decode a JSON string into a Lua value. Returns (nil, err) on a parse error.
---@param json string
---@return any? value
---@return string? err
function dcs_studio_gui_json.decode(json) end

--- TOML encode/decode helpers (bridged through JSON).
---@class dcs_studio_gui.toml
local dcs_studio_gui_toml = {}
...
---@return string? toml
---@return string? err
function dcs_studio_gui_toml.encode(value) end

--- Decode a TOML string into a Lua table. Returns (nil, err) on a parse error.
---@param toml string
---@return table? value
---@return string? err
function dcs_studio_gui_toml.decode(toml) end

--- Write sim data to disk under the guarded DCS write root (lfs.writedir()).
---@class dcs_studio_gui.file
local dcs_studio_gui_file = {}
...
---@return boolean? ok
---@return string? err
function dcs_studio_gui_file.write_text(path, content, opts) end
...
---@return boolean? ok
---@return string? err
function dcs_studio_gui_file.write_json(path, value, opts) end
...
---@return boolean? ok
---@return string? err
function dcs_studio_gui_file.write_csv(path, rows) end
...
---@return boolean? ok
---@return string? err
function dcs_studio_gui_file.dump(path, value, opts) end

--- An open SQLite database handle.
---@class dcs_studio_gui.sqlite.Db
local dcs_studio_gui_sqlite_Db = {}
...
---@return string? err
function dcs_studio_gui_sqlite_Db:exec(sql, params) end
...
---@param path string
---@return dcs_studio_gui.sqlite.Db? db
---@return string? err
function dcs_studio_gui_sqlite.open(path) end

--- Sim→IDE console pipe: printed lines stream into the DCS Studio Console panel.
---@class dcs_studio_gui.console
local dcs_studio_gui_console = {}
...
---@param lines number[]
---@return number count
function dcs_studio_gui_debug.set_breakpoints(source, lines) end

--- Whether a breakpoint is set at `source:line` — consulted by the sim's line hook.
---@param source string
---@param line number
---@return boolean paused
function dcs_studio_gui_debug.should_pause(source, line) end

--- Remove every breakpoint.
function dcs_studio_gui_debug.clear_breakpoints() end
...
---@param ms number
function dcs_studio_gui_debug.sleep_ms(ms) end
...
function dcs_studio_gui_debug.reset_session() end

--- A namespaced logger writing to the DCS Studio log.
---@class dcs_studio_gui.logger.Logger
local dcs_studio_gui_logger_Logger = {}

--- Create a logger that tags every line with namespace `ns`.
---@param ns string
---@return dcs_studio_gui.logger.Logger
function dcs_studio_gui_logger_Logger.new(ns) end

--- Log at debug level under this logger's namespace.
---@param msg string
function dcs_studio_gui_logger_Logger:debug(msg) end

--- Log at info level under this logger's namespace.
---@param msg string
function dcs_studio_gui_logger_Logger:info(msg) end

--- Log at warn level under this logger's namespace.
---@param msg string
function dcs_studio_gui_logger_Logger:warn(msg) end

--- Log at error level under this logger's namespace.
---@param msg string
function dcs_studio_gui_logger_Logger:error(msg) end

--- Namespaced logging into the DCS Studio log file.
---@class dcs_studio_gui.logger
---@field Logger dcs_studio_gui.logger.Logger # A namespaced logger writing to the DCS Studio log.
local dcs_studio_gui_logger = {}

--- Log a message at debug level.
---@param msg string
---@param ns? string
function dcs_studio_gui_logger.debug(msg, ns) end

--- Log a message at info level.
---@param msg string
---@param ns? string
function dcs_studio_gui_logger.info(msg, ns) end

--- Log a message at warn level.
---@param msg string
---@param ns? string
function dcs_studio_gui_logger.warn(msg, ns) end
...
--- Bind a server. `config = { host = string, port = number, timeout? = number, env? = string, pump_stale_ms? = number }`. timeout is the request deadline in seconds (default 30; 0 means effectively never, for interactive debugging). pump_stale_ms is how long this server's queue may go undrained before arriving requests are refused with -32002 'sim not pumping' instead of queueing into the request timeout (default 2000; 0 disables). -32002 also carries 'queue full', the bridge's other refusal: the queue holds 256 entries, and a request arriving at a full one is answered rather than queued. A NOTIFICATION is never refused this way, because it has no id to answer to — at a full queue the oldest queued notification is dropped to make room for it, and if every entry is a request with a caller waiting then the arriving notification is the one dropped. Either way the drop is logged at warn and reported nowhere else. The same thing `serve` does — prefer `serve`, which is what both bridges' boot code calls.
---@param config table
...
local dcs_studio_gui_jsonrpc_JsonRpcRouter = {}

--- Create an empty router.
---@return dcs_studio_gui.jsonrpc.JsonRpcRouter
function dcs_studio_gui_jsonrpc_JsonRpcRouter.new() end
...
---@param meta? table
function dcs_studio_gui_jsonrpc_JsonRpcRouter:add_method(name, handler, meta) end
...
---@field json dcs_studio_gui.json # JSON encode/decode helpers.
---@field toml dcs_studio_gui.toml # TOML encode/decode helpers (bridged through JSON).
---@field file dcs_studio_gui.file # Write sim data to disk under the guarded DCS write root (lfs.writedir()).
---@field sqlite dcs_studio_gui.sqlite # Embedded SQLite — open/query a database under the guarded write root.
---@field console dcs_studio_gui.console # Sim→IDE console pipe: printed lines stream into the DCS Studio Console panel.
---@field debug dcs_studio_gui.debug # Breakpoint registry the IDE debugger drives over the bridge.
---@field logger dcs_studio_gui.logger # Namespaced logging into the DCS Studio log file.
---@field jsonrpc dcs_studio_gui.jsonrpc # The WebSocket/HTTP JSON-RPC server and router.
local dcs_studio_gui = {}

--- Return the generated EmmyLua (.d.lua) type definitions for this module.
---@return string
function dcs_studio_gui.emit_dlua() end
...
---@return string
function dcs_studio_gui.dump_globals() end
...
---@param deps table
---@return table
function dcs_studio_gui.register_methods(router, deps) end

return dcs_studio_gui
